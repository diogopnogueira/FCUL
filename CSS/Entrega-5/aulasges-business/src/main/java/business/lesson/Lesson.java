...
@Entity
@NamedQueries({
		@NamedQuery(name = Lesson.FIND_BY_NAME, query = "SELECT l FROM Lesson l WHERE l.name = :" + Lesson.LESSON_NAME),
		@NamedQuery(name = Lesson.FIND_ACTIVE_BY_SPORTMODALITY, query = "SELECT l FROM Lesson l WHERE l.status = :"
				+ Lesson.LESSON_STATUS + " AND l.sportModality = :" + Lesson.LESSON_SPORTMODALITY
				+ " ORDER BY l.startHour") })
public class Lesson {
	...
	public Lesson(String name, SportModality sportModality, String[] daysOfWeek, LocalTime startHour, int duration) {
		this.name = name;
